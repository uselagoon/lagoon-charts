...
ifeq ($(INGRESS_CONTROLLER),traefik)
	$(HELM) upgrade \
		...
		ingress-traefik \
		traefik/traefik
	export INGRESS_IP="$$($(KUBECTL) -n ingress-traefik get services ingress-traefik -o jsonpath='{.status.loadBalancer.ingress[0].ip}')" && \
...
ifeq (,$(subst ",,$(STABLE_BUILDDEPLOY_CHART_VERSION)))
	...
	$(HELM) upgrade \
		...
		--set "QoSMaxBuilds=5" \
		--set "extraEnvs[0].name=LAGOON_FEATURE_FLAG_DEFAULT_INGRESS_CLASS,extraEnvs[0].value=$(INGRESS_CONTROLLER_CLASSNAME)" \
