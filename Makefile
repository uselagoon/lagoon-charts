...
install-lagoon-core: install-calico
	$(HELM) upgrade \
		...
		--set logs2email.image.repository=$(IMAGE_REGISTRY)/logs2email \
		--set logs2microsoftteams.image.repository=$(IMAGE_REGISTRY)/logs2microsoftteams \
		--set logs2rocketchat.image.repository=$(IMAGE_REGISTRY)/logs2rocketchat \
		--set logs2slack.image.repository=$(IMAGE_REGISTRY)/logs2slack \
		--set logsDBCurator.image.repository=$(IMAGE_REGISTRY)/logs-db-curator \
		--set localAPIDataWatcherPusher.image.repository=$(IMAGE_REGISTRY)/local-api-data-watcher-pusher \
		--set ssh.image.repository=$(IMAGE_REGISTRY)/ssh \
		--set sshPortal.enabled=true \
